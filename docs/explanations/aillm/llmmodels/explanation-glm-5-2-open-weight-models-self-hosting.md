...
```text
From Hosted-Model Pull to Open-Weight Self-Hosting

Fable 5 / Mythos 5 pulled worldwide
  June 12 (US export-control directive, no grace period)

GLM-5.2 ships same week
  Z.ai: MIT-licensed open weights, 753B MoE, 1M-token context

Lesson: access is not ownership
  enterprises on Fable lost their engine in an afternoon

Self-hosting economics
  ~$20k local hardware (DGX Spark)
  pays back vs API bills in 6-7 mo
  swap models via config, not rewrite
```
