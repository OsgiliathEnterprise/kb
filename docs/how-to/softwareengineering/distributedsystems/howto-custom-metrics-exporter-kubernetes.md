...
A step-by-step guide to writing a custom Prometheus metrics exporter in Go, containerizing it, deploying it to Kubernetes, and wiring it to Prometheus for custom metric-based autoscaling via the HorizontalPodAutoscaler.

## What a Metrics Exporter Does

An exporter is a lightweight HTTP server that exposes application state as plain text on a `/metrics` endpoint. Prometheus scrapes this endpoint at regular intervals, stores the time-series data, and makes it available for queries, alerts, and autoscaling rules.

**When to use a standalone exporter vs. embedding metrics:**
- **Standalone exporter**: When the data source is external to your application or you don't control the application code
- **Embedded metrics**: When you can add the Prometheus client library directly to your application

## Prometheus Metric Types

| Type | Description | Example Use Case |
|---|---|---|
| **Counter** | Monotonically increasing values | Total requests served, errors encountered |
| **Gauge** | Values that rise and fall | Queue depth, active connections, cache size |
...
`prometheus.MustRegister` panics on duplicate registration, making misconfigurations fail fast at startup. For library use, prefer `prometheus.Register` with error handling.
...
**Polling interval guidance**: Keep it shorter than Prometheus's scrape interval (default 15s) so each scrape sees fresh data.
...
`distroless/static:nonroot` has no shell, no package manager, and runs as non-root — satisfying most cluster security policies.
...
Navigate to `http://localhost:9090/targets` — the exporter should show as **UP**.
...
- [HorizontalPodAutoscaler Documentation](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale-walkthrough/)
